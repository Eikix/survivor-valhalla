...
mod test_energy_actions {
    ...
    use survivor_valhalla::systems::battle_actions::{
        IBattleActionsDispatcher, IBattleActionsDispatcherTrait,
    };
    ...
    use survivor_valhalla::systems::energy_actions::{
        IEnergyActionsDispatcher, IEnergyActionsDispatcherTrait,
    };
    use survivor_valhalla::tests::test_world::{
        deploy_battle_actions, deploy_beast_actions, deploy_energy_actions, setup_world,
    };

    const PLAYER1: felt252 = 'PLAYER1';

    fn setup_energy_test() -> (
        WorldStorage, IEnergyActionsDispatcher, IBeastActionsDispatcher, ContractAddress,
    ) {
    ...
    fn test_multiple_players_energy() {
        ...
        assert(energy_actions.get_energy(player1) == 2, 'T_E_P1_2');
        assert(energy_actions.get_energy(player2) == 4, 'T_E_P2_4');
    }
}
